...
class RpsClass
{
    ...
    /**
     * Set CodigoPais of RPS
     * @param string $value
     * @throws InvalidArgumentException
     */
    public function codigoPaisBacen($value)
    {
        $value = trim($value);
        if (!Validator::stringType()->length(1, 4)->validate($value)) {
            throw new \InvalidArgumentException('O Código do Pais não pode ser vazio deve ter até 4 caracteres.');
        }
        $this->codPaisExterior = $value;
    }

    public function codigo_nif($value){
        $value = trim($value);
        if (!Validator::stringType()->length(1, 50)->validate($value)) {
            throw new \InvalidArgumentException('O Código NIF não pode ser vazio deve ter até 50 caracteres.');
        }
        $this->nif = $value;
    }
     ...
     * @param DateTime $value
     */
    public function dataCompetencia(DateTime $value)
    {
        $this->infDataCompetencia = $value;
    }
    ...
    {
        if (!Validator::numeric()->intVal()->between(1, 6)->validate($value)) {
            throw new \InvalidArgumentException('O regime de tributação deve estar entre 1 e 6.');
        }
    ...
    public function codigoNbs($value){
        $value = trim($value);
        if (!Validator::stringType()->length(1, 20)->validate($value)) {
            throw new \InvalidArgumentException('O Código de NBS obrigatório e deve ter '
                    . 'no máximo 20 caracteres.');
        }
    ...
    /**
     * Set complementary complementary informations of service
     * @param string $value
     * @throws InvalidArgumentException
     */
    public function informacoesComplementares($value)
    {
        $value = trim($value);
        if (!Validator::stringType()->length(1, 2000)->validate($value)) {
            throw new \InvalidArgumentException('A informação complementar deve ter '
                    . 'no máximo 2000 caracteres.');
        }
     ...
     * @throws InvalidArgumentException
     */
    public function municipioIncidencia($value)
    {
        if (!Validator::numeric()->intVal()->validate($value)) {
            throw new \InvalidArgumentException('Deve ser passado o código do IBGE.');
        }
    ...
    public function valorTotalTributos($value = 0.00)
    {
        if (!Validator::numeric()->floatVal()->min(0)->validate($value)) {
            throw new \InvalidArgumentException('Os valores deve ser numericos tipo float.');
        }
        $this->valorTotalTributos = round($value, 2);
    }
