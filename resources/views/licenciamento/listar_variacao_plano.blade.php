<x-area-cliente-layout title="Renovação de Licença de Uso">
    <x-slot name="header">
                ...
                <div class="max-w-sm p-8 border-t-4 border-{{ $cores[$chaveCor] }}-600 rounded shadow-lg">
                    ...
                    <div>
                        ...
                        <div class="flex items-center justify-center mt-4">
                            <a href="{{ route('licenca.gerar-fatura', base64_encode($vPlano->id)) }}" class="text-center w-full px-2 py-2 text-2xl text-{{ $cores[$chaveCor] }}-200 bg-{{ $cores[$chaveCor] }}-600 rounded">Renovar {{$vPlano->descricao}}</a>
                        </div>
                    </div>
                </div>
            @endforeach
